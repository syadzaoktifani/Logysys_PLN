...
// State Bon Pinjam Realtime Spreadsheet & Filter
  ...
  const handleSendChat = async (e) => {
    ...
    try {
      const response = await fetch('https://logysys-pln.vercel.app/api/gemini-chat', {
        method: 'POST',
  ...
  useEffect(() => {
    const fetchTopMaterials = async () => {
      setIsSyncing(true);
      try {
        const response = await fetch('https://logysys-pln.vercel.app/api/top-materials');
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        // Fetch Top Materials
        const resTop = await fetch('https://logysys-pln.vercel.app/api/top-materials');
        ...
        const resBon = await fetch('https://logysys-pln.vercel.app/api/bon-pinjam');
...
const handleSaveEdit = async (e) => {
  ...
  try {
    const response = await fetch('https://logysys-pln.vercel.app/api/update-bon-pinjam', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
      ...
      <main className="flex-1 min-h-[calc(100vh-5rem)]">
        ...
        <section id="beranda" className="relative w-full h-[calc(100vh-5rem)] md:h-[calc(100dvh-5rem)] max-w-[100vw] overflow-hidden group">
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center px-4 sm:px-6 z-10">
            <h2 className="text-3xl sm:text-4xl md:text-6xl font-extrabold text-white text-center drop-shadow-lg mb-6 leading-tight transition-all">
              Sistem Informasi Manajemen Logistik <br />PLN UP3 Padang
            </h2>
